  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 mb-4 min-h-[100px]">
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
            ...
            {m.role === 'assistant' && (
              <div className="w-6 h-6 rounded-full bg-gold-400/10 border border-gold-400/20 flex items-center justify-center shrink-0 mt-0.5">
                <Volume2 size={10} className={cn('text-gold-400', isSpeaking && i === conversation.length - 1 && 'animate-pulse')} />
              </div>
            )}
        ...
        {error && (
          <p className="text-xs text-ember-400 text-center">{error}</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 flex items-center gap-0.5 h-8">
          {bars.map((_, i) => (
            <motion.div key={i}
              animate={isListening ? { height: [`${20 + Math.sin(i) * 15}%`, `${40 + Math.sin(i * 1.3 + 1) * 30}%`, `${20 + Math.sin(i) * 15}%`] } : { height: `${15 + Math.sin(i * 0.8) * 10 + 10}%` }}
              transition={{ duration: 0.5, repeat: isListening ? Infinity : 0, delay: i * 0.04 }}
              className={cn('flex-1 rounded-full', isListening ? 'bg-ember-400' : isSpeaking ? 'bg-gold-400' : 'bg-obsidian-600')}
            />
          ))}
        </div>

        <div className="text-[10px] text-gray-600 shrink-0">
          {!isSupported ? 'Chrome only' : isListening ? 'Release to send' : isThinking ? 'Processing…' : 'Hold to speak'}
        </div>
      </div>
    </div>
  )
